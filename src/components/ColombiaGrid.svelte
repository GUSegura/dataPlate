...
<!-- <script>
    ...
    onMount(async () => {
      try {  
        ...
        color = data.map((d)=>"#585858");
        // x_pos = data.map((d)=>0.5);
        // y_pos = data.map((d)=>0.5);
        // size = data.map((d)=>1);
    ...
    function updateStep(step) {
      if (step == 0) {
        ...
        color = data.map((d)=>"#585858");
        // x_pos = data.map((d)=>data.map((d,i)=>i % numCols));
        // y_pos = data.map((d)=>Math.floor(i / numCols));
        // size = data.map((d)=>circleSize / 2);
      }
      if (step == 1) {
        data.forEach(d => { d.color = d.students_in_hh > 0 ? "#0595b3" : "gray" });
      ...
      if (step == 2) {
        ...
        color = data.map((d)=>data.map((d)=>{return d.students_in_hh > 0 ? d.recieves_meal_plan > 0 ? "#A491D3" : "#0595b3" : "#585858"}));
        // x_pos = data.map((d)=>data.map((d,i)=>i % numCols));
        // y_pos = data.map((d)=>Math.floor(i / numCols));
        // size = data.map((d)=>circleSize / 2);
      }
      if (step == 3) {
        ...
        color = data.map((d)=>{return d.recieves_meal_plan > 0 ? "#A491D3" : "#f9f5f1"});
        // x_pos = data.map((d)=>data.map((d,i)=>i % numCols));
        // y_pos = data.map((d)=>Math.floor(i / numCols));
        // size = data.map((d)=>circleSize / 2);
      }
      if (step == 4) {
        ...
        color = data.map((d)=> d.recieves_meal_plan > 0);
        // x_pos = data.map((d)=>data.map((d,i)=>i % numCols));
        // y_pos = data.map((d)=>Math.floor(i / numCols));
        // size = data.map((d)=>circleSize / 2);
      }
